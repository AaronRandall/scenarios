  ...
  module Devices
    ...
    class Hardware < TestDevice
      ...
      def install_app
        ...
        system "#{SCRIPT_PATH}/transporter_chief.rb #{@hardware_id.nil? ? '' : "--device " + @hardware_id} #{@ops.ios_app_path}/build/Debug-iphoneos/#{@ops.ios_app_name}.app"
      end
      ...
      def run_tests
        @tests_to_run.each do |test|
          ...
          command_to_run = "#{TUNEUP_BIN} #{@ops.ios_app_name} #{test} #{OUTPUT_PATH} -d #{@hardware_id.nil? ? 'dynamic' : @hardware_id} -p -j #{username_password_parameter}"
